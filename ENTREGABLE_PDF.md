---
title: "Backend E-commerce + Módulo Adoption"
subtitle: "Entregable Final — CoderHouse Backend 3"
author: "Miguel Zambrano — Comisión 76985"
date: "Junio 2026"
---
...
Los archivos marcados con 🆕 son los creados específicamente para este entregable.

## 1.3 Propósito de archivos y carpetas principales
...
| `src/utils/mailer.js` | Nodemailer con fallback a `jsonTransport`. |
    ...
    describe('PUT /api/adoptions/:aid (admin)', () => {
        it('should require authentication', async () => {
            const response = await request(app)
                ...
                .send({ description: 'Animal con descripción actualizada' });
            expect(response.status).toBe(401);
        });
...
## 2.4 Evidencia de ejecución de tests

Comando: `node --experimental-vm-modules node_modules/jest/bin/jest.js --detectOpenHandles --forceExit`
...
|---|---|
| **`node:20-alpine`** como base | Imagen ligera (~120 MB vs ~900 MB de Debian/Ubuntu). |
| **Multi-stage build** (`builder` + `production`) | Descarta devDependencies en la imagen final. Reduce superficie de ataque. |
| **`npm install --omit=dev`** | Evita instalar jest, supertest, mongodb-memory-server en producción. |
| **`npm cache clean --force`** | Elimina caché de npm para reducir capas y tamaño. |
| **`COPY --from=builder`** | Reutiliza dependencias ya instaladas de la etapa builder. |
| **`USER node`** | Ejecuta como usuario no root (buena práctica de seguridad). |
| **`HEALTHCHECK`** con `wget` | Permite a Docker/orquestadores monitorear el contenedor. |
| **`LABEL` con metadatos OCI** | Etiquetas estándar para registro de imágenes. |
| **`CMD` en formato exec** | Señales del kernel llegan correctamente a Node. |
...
#5 [builder 1/4] FROM docker.io/library/node:20-alpine@sha256:...
#5 DONE 4.9s
...
#15 exporting manifest sha256:18a70aa...
#15 exporting config sha256:e53c806...
...
## 4.2 Evidencia de construcción correcta

La imagen fue construida exitosamente usando `docker build -t backend-ecommerce:1.0.0 .` con un build multi-stage que completó todas las etapas sin errores. El log de construcción (sección 3.3) muestra:

- **Builder stage**: 158 packages instalados, 5.6s de duración
- **Production stage**: Dependencias copiadas, archivos de source copiados, permisos asignados
- **Export**: Layers, manifest, config y naming completados en 3.8s
- **Tamaño final**: 315 MB

## 4.3 Evidencia de ejecución del contenedor

Comando:

```bash
docker run -d --rm --name backend-ecommerce-test \
  -p 8081:8080 \
  -e MONGODB_URI=mongodb://127.0.0.1:27017/test \
  -e JWT_PRIVATE_KEY=clave-secreta-de-prueba-min-32-caracteres-long \
  -e PORT=8080 \
...
Test funcional contra el contenedor:

```bash
curl -s http://localhost:8081/api/adoptions
```

Respuesta (esperada — sin MongoDB disponible en el host):

```json
{"status":"error","message":"Operation `adoptions.find()` buffering timed out after 10000ms"}
```

El router `/api/adoptions` está montado, Express responde, y la app maneja errores con JSON estructurado.
...
API REST para e-commerce con un módulo adicional de **gestión de adopciones de mascotas**,
desarrollado con Node.js, Express y MongoDB. Incluye autenticación con JWT, manejo de
carritos, productos y un CRUD completo de adopciones con tests funcionales y Docker.

## Estructura del Proyecto

Ver sección 1 para árbol completo.
...
| MONGODB_URI | URI de conexión MongoDB | mongodb://localhost:27017/ecommerce |
| JWT_PRIVATE_KEY | Clave secreta JWT (mín 32 chars) | — |
...
## Endpoints principales — Adopciones (/api/adoptions)
...
| Requisito | Estado |
|---|---|
| Estructura del proyecto documentada con árbol y propósito de archivos | ✅ |
| Tests funcionales de `adoption.router.js` (21/21 passing) | ✅ |
| Suite completa de tests (55/55 passing) | ✅ |
| Dockerfile multi-stage optimizado | ✅ |
| docker-compose.yml con MongoDB | ✅ |
| Imagen Docker construida: `backend-ecommerce:1.0.0` (315 MB) | ✅ |
| Contenedor ejecutado y validado (logs capturados) | ✅ |
| README actualizado con instrucciones reproducibles | ✅ |
| Documento final con todas las secciones requeridas | ✅ |

---

**Autor**: Miguel Zambrano  
**Comisión**: 76985  
**Curso**: CoderHouse Backend 3  
**Fecha**: Junio 2026
